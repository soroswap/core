...
fn try_withdraw_not_yet_deposited() {
    ...
    assert_eq!(
        result,
        Err(Ok(SoroswapPairError::WithdrawLiquidityNotInitialized))
    );
}
...
fn try_withdraw_not_shares_sent() {
    ...
    assert_eq!(
        result,
        Err(Ok(SoroswapPairError::WithdrawInsufficientSentShares))
    );
}
...
fn withdraw() {
    ...
    assert_eq!(
        ...
        original_total_supply_0.checked_sub(1000).unwrap()
    );
    assert_eq!(
        test.token_1.balance(&test.user),
        original_total_supply_1.checked_sub(1000).unwrap()
    );
}
