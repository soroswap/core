...
fn sort_tokens_same_address() {
    ...
    let address_0 = Address::from_string(&String::from_str(
        &test.env,
...
fn sort_tokens_correct_order() {
    ...
    let address_1 = Address::from_string(&String::from_str(
        &test.env,
        "CDMLFMKMMD7MWZP3FKUBZPVHTUEDLSX4BYGYKH4GCESXYHS3IHQ4EIG4",
    ));
    assert_eq!(
        (address_0.clone(), address_1.clone()),
        test.contract.sort_tokens(&address_0, &address_1)
    );
    assert_eq!(
        (address_0.clone(), address_1.clone()),
        test.contract.sort_tokens(&address_1, &address_0)
    );
}
