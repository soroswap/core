...
fn get_amount_out() {
    ...
    //  Add Liquidity:
    test.token_0
        .transfer(&test.user, &test.pair.address, &amount_0);
    test.token_1
        .transfer(&test.user, &test.pair.address, &amount_1);
    ...
    let expected_output_amount_1 =
        test.contract
            .get_amount_out(&swap_amount_0, &amount_0, &amount_1);
    test.token_0
        .transfer(&test.user, &test.pair.address, &swap_amount_0);
...
fn get_amount_out_insufficient_input_amount() {
    ...
    assert_eq!(
        result,
        Err(Ok(SoroswapLibraryError::InsufficientInputAmount))
    );
}
...
fn get_amount_in() {
    ...
    let swap_required_amount_0: i128 =
        test.contract
            .get_amount_in(&swap_expected_output_amount_1, &amount_0, &amount_1);
    test.token_0
        .transfer(&test.user, &test.pair.address, &swap_required_amount_0);
    test.pair
        .swap(&0, &swap_expected_output_amount_1, &test.user);
...
fn get_amount_in_insufficient_output_amount() {
    ...
    assert_eq!(
        result,
        Err(Ok(SoroswapLibraryError::InsufficientOutputAmount))
    );
}
...
fn get_amounts_out() {
    ...
    test.token_0
        .transfer(&test.user, &test.pair.address, &10000);
    test.token_1
        .transfer(&test.user, &test.pair.address, &10000);
    test.pair.deposit(&test.user);

    let expected_amounts_out = vec![&test.env, 3, 1];
    let amounts_out = test
        .contract
        .get_amounts_out(&test.factory.address, &3, &path);
...
fn get_amounts_out_invalid_path() {
    ...
    let result = test
        .contract
        .try_get_amounts_out(&test.factory.address, &2, &path);
...
fn get_amounts_in() {
    ...
    let amounts_out = test
        .contract
        .get_amounts_in(&test.factory.address, &1, &path);
...
fn get_amounts_in_invalid_path() {
    ...
    let result = test
        .contract
        .try_get_amounts_in(&test.factory.address, &1, &path);
    assert_eq!(result, Err(Ok(SoroswapLibraryError::InvalidPath)));
}
