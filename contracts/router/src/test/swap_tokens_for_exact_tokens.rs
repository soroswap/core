use soroban_sdk::{testutils::Ledger, vec, Address, Vec};
...
fn swap_tokens_for_exact_tokens_amount_in_should() {
    ...
    assert_eq!(
        ...
        original_balance - amount_0 - expected_amount_0_in
    );
    assert_eq!(
        test.token_1.balance(&test.user),
        original_balance - amount_1 + expected_amount_out
    );
    ...
    assert_eq!(
        test.token_0.balance(&pair_address),
        amount_0 + expected_amount_0_in
    );
    assert_eq!(
        test.token_1.balance(&pair_address),
        amount_1 - expected_amount_out
    );
}
...
fn swap_tokens_for_exact_tokens() {
    ...
    assert_eq!(
        test.token_0.balance(&test.user),
        original_balance - amount_0 - amount_in_should
    );
    assert_eq!(
        test.token_1.balance(&test.user),
        original_balance - amount_1 + expected_amount_out
...
fn swap_tokens_for_exact_tokens_2_hops() {
    ...
    assert_eq!(
        test.token_1.balance(&test.user),
        original_balance - amount_1 * 2
    );
    assert_eq!(
        token_2.balance(&test.user),
        original_balance - amount_2 + expected_amount_out
    );

    let pair_address_0_1 = test
        .factory
        .get_pair(&test.token_0.address, &test.token_1.address);
    assert_eq!(
        test.token_0.balance(&pair_address_0_1),
        amount_0 + amount_in_should
    );
    assert_eq!(
        test.token_1.balance(&pair_address_0_1),
        amount_1 - middle_amount_in
    );

    let pair_address_1_2 = test
        .factory
        .get_pair(&test.token_1.address, &token_2.address);
    assert_eq!(
        test.token_1.balance(&pair_address_1_2),
        amount_1 + middle_amount_in
    );
    assert_eq!(
        token_2.balance(&pair_address_1_2),
        amount_2 - expected_amount_out
    );
}
