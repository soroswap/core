...
use crate::test::{SoroswapPairClient, SoroswapRouterTest};
...
fn test_remove_liquidity_equal_amount_0_minimum_out() {
    ...
    assert_eq!(
        ...
        initial_user_balance.checked_sub(amount_0).unwrap()
    );
    assert_eq!(
        test.token_1.balance(&test.user),
        initial_user_balance.checked_sub(amount_1).unwrap()
    );
    ...
    let expected_liquidity: i128 = expected_total_liquidity
    ...
    assert_eq!(
        removed_0,
        (amount_0 * expected_liquidity) / expected_total_liquidity
    );
    assert_eq!(
        removed_1,
        (amount_1 * expected_liquidity) / expected_total_liquidity
    );
    ...
    assert_eq!(
        test.token_0.balance(&test.user),
        initial_user_balance.checked_sub(locked_0).unwrap()
    );
    assert_eq!(
        test.token_1.balance(&test.user),
        initial_user_balance.checked_sub(locked_1).unwrap()
    );
