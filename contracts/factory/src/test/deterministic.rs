...
impl<'a> SoroswapFactoryTest<'a> {
    fn new() -> Self {
        ...
        let mut token_0: TokenClient<'a> =
            TokenClient::new(&env, &env.register_stellar_asset_contract(alice.clone()));
        let mut token_1: TokenClient<'a> =
            TokenClient::new(&env, &env.register_stellar_asset_contract(alice.clone()));
        if &token_1.address < &token_0.address {
            mem::swap(&mut token_0, &mut token_1);
        } else if &token_1.address == &token_0.address {
...
pub fn pair_exists_both_directions() {
    ...
    assert_eq!(
        factory.pair_exists(&token_0.address, &token_1.address),
        true
    );
    assert_eq!(
        factory.pair_exists(&token_1.address, &token_0.address),
        true
    );
}
...
pub fn pair_does_not_exists_both_directions() {
    ...
    let token_a = TokenClient::new(
        &factory.env,
        &factory.env.register_stellar_asset_contract(alice.clone()),
    );
    let token_b = TokenClient::new(
        &factory.env,
        &factory.env.register_stellar_asset_contract(alice.clone()),
    );
    assert_eq!(
        factory.pair_exists(&token_a.address, &token_b.address),
        false
    );
    assert_eq!(
        factory.pair_exists(&token_b.address, &token_a.address),
        false
    );
}
...
pub fn pair_address_eq_both_directions() {
    ...
    let a = factory_test
        .factory
        .get_pair(&token_0_address, &token_1_address);
    let b = factory_test
        .factory
        .get_pair(&token_1_address, &token_0_address);
    assert_eq!(a, b)
}
...
pub fn compare_deterministic_address() {
    ...
    let calculated_pair_address = env
        .deployer()
        .with_address(
            factory_test.factory.address.clone(),
            bytes_n_32_salt.clone(),
        )
        .deployed_address();
