...
fn create_pair_one_way() {
    ...
    assert_eq!(
        test.contract
            .pair_exists(&test.token_1.address, &test.token_0.address),
        false
    );
...
fn double_pair_creation_other_way() {
    ...
    let res = test
        .contract
        .try_create_pair(&test.token_1.address, &test.token_0.address);
...
fn create_identical_tokens() {
    ...
    let res = test
        .contract
        .try_create_pair(&test.token_0.address, &test.token_0.address);
